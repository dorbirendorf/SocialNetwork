    ...
    return (
        <div className="post bg-white p-1 my-1">
          ...
          <div>
            ...
            {showActions&&<button type="button" className="btn btn-light" onClick={(e)=>removeLike(_id)}>
              <i className="fas fa-thumbs-down"></i>
            </button>}
            {showActions&&<Link to={`posts/${_id}`} className="btn btn-primary">
              Discussion {comments.length>0 && <span className='comment-count'>{comments.length}</span>}
            </Link>}
            {!auth.loading &&user===auth.user._id && showActions &&<button      
            type="button"
